...
The data for this analysis was downloaded with the [NLS Investigator](https://www.nlsinfo.org/investigator/pages/login) and can be found [*here*.]{.ul} The SES at age 40 data can be found [*here*]{.ul}. For clarity, and to emphasize the functionality of {discord}, the data has been pre-processed using [*this script*]{.ul}. This discordant-kinship analysis is possible thanks to recent work that estimated relatedness for approximately 95% of the NLSY79 kin pairs [@rodgers2016]. These kinship links are included in the [{NlsyLinks}](http://nlsy-links.github.io/NlsyLinks/index.html) R package [@beasley2016] and are easily utilized with the {discord} package.
...
```{r preview-pre-processed-data, include = FALSE}
flu_ses_data %>%
...
```{r preview-linked-dat, include = FALSE}
df_link %>%
...
The default output of `discord_regression()` is a tidy data frame containing the model metrics -- courtesy of the [`broom`](https://broom.tidymodels.org) package [@robinson2021]. In this example, our results are:
...
Coming soon to a website in your browser...
